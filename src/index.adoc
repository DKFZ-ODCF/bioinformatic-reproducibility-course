...
== Workflow Management Systems
  ...
  * Workflow Management Systems
    ...
    * Log execution to easy reproduction
...
== Some Notes on "Programming"
  ...
  * Communication
    ...
    - Others must understand your code
  * Complexity management
    - Biological systems are complex
    - Bioinformatic code to analyze biological systems is complex
    - Complexity increases with the time you add analyses

=== Programming Languages

  * Writing a workflow is programming
  * Log in to a computer and you'll get a shell
    - Doing s.th. on the shell is programming
  * Understand
...
==== Bash
  ...
  * Bad for
    ...
    - Workflows with more than 2-3 steps
...
==== Python, Perl, Ruby or other Scripting Languages
  ...
  * Good for
    ...
    - Get going quickly
  * Bad for
    - Really fast processing (except numerics)
...
=== Programming Power Tools
  ...
  * Use an IDE (PyCharm, IntelliJ IDEA, ...)
  * Automated tests
...
== Version Control Systems
  ...
  * Manage many versions of your code
...
> is_dirty <- function(status) {
    length(status$unstaged) != 0 || length(status$untracked) != 0 # <3>
}
> if (is_dirty(status(repo))) { stop("Not proceeding! Repo is dirty!"); } # <4>
> commitHash <- sha(head(repo)) # <5>
----
<1> Load the R library for accessing git repositories (you can get it from https://cran.r-project.org/web/packages/git2r/index.html[CRAN])
<2> Get a handle for the repository
<3> Definition of "dirty": there are unstaged changes or untracked files
...
Simply add the `commitHash` to your figures, output directories, filenames, or similar.
